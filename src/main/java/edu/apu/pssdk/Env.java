...
public class Env {
  ...
  public static Ci ciFactory(String ciName, Map<String, Boolean> options) throws JOAException {
    ...
    if (Strings.isNullOrEmpty(strServerName)
      ...
      throw new JOAException("Connect information provided is incomplete. Please provide all necessary environment variables.");
    }
    ...
    if (Strings.isNullOrEmpty(strDomainConnPswd)) {
      ...
      isConnected = iSession.connectS(1, strAppServerPath, strUserID, strPassword, null, strDomainConnPswd);
    }
    if (!isConnected) {
    ...
    return session.getCompIntfc(ciName, options);
  }
}
